...
We draw inspiration from software development: programmers write tests for intermediate steps to flag potential issues and use the feedback to improve the software. We treat the structured policy similarly to develop through this iterative refinement process (Figure~\ref{fig:overview}).
...
We use the superscript $(i)$ to denote the policy after $i$ iterations.
...
Formally, given a demonstration as a set of state-action pairs,
\begin{equation}
...
\end{minipage}\hfill
\begin{minipage}[t]{.485\linewidth}
\centering\textbf{\small (b) After: centering amplified when off-track}\\[3pt]
\resizebox{\linewidth}{!}{\input{figures/diagrams/incremental_after}}
\end{minipage}
\par\vspace{5pt}
\begin{minipage}[t]{.485\linewidth}
\centering\textbf{\small (c) Recovery behavior analysis}\\[6pt]
...
\end{minipage}\hfill
\begin{minipage}[t]{.485\linewidth}
\centering\textbf{\small (d) Recorded lateral offset and steering}\\[3pt]
...
(a) The steering structure before refinement (other nodes omitted);
(b) The refined structure adds stronger steering when the race car is off-track;
(c) The desired behavior of re-centering, corresponding test, and the result;
(d) Recorded lateral offset and steering.
}
...
Given the task description and current structure $\llbracket \alpha^{(i)} \rrbracket$, the LLM proposes hypotheses and writes Python analysis functions $T^{(i)}_j$ to test each of the hypotheses.

Figure~\ref{fig:latent-traces}(c) illustrates one hypothesis: the policy should attempt to recenter after the car goes off-track.
...
The test directly queries relevant variables (e.g., \texttt{offset}). We take advantage of the flexibility of Python syntax and have no limit on what kind of tests it can generate.
The above example filters off-track time steps and measures the percentage of time steps in which the policy exhibits recovery behavior.
...
Figure~\ref{fig:latent-traces}(d) plots these two recorded columns over the full 1,000-step rollout.
...
In Figure~\ref{fig:latent-traces}(c), the test finds center-directed steering in only $14.9\%$ of off-track steps.
...
Figure~\ref{fig:latent-traces}(b) shows the recovery branch after the revision.
...
Convergence in the policy structure $\llbracket \alpha \rrbracket$ indicates no structural improvement is needed for the policy.
To further improve performance, we apply a finite-length variant of GSPO \citep{zheng2025group} reinforcement learning to further optimize the continuous parameters $\Theta$.
Specifically, given $k$ trajectories per step, we use a sliding window of length $l$ to break each of them into tracklets of length $l$, then form groups such that the starting positions of the groups are similar.
Appendix~\ref{app:rl} describes the implementation details.